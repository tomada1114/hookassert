...
| Per-hook `timeout` — the docs define it only as "Seconds before canceling" and say nothing about zero, negative, or fractional values                                                                                         | A zero or negative `timeout` would multiply out to an instant (or negative) millisecond timeout at execution, and every affected hook fails as a timeout with no hint that the settings file is the cause. Rejected at settings load, before `lint`'s rules ever run: `"hooks.<event>[].hooks[].timeout" at line N must be a positive number of seconds, got <value>` (`SettingsParseError`, exit 5). Fractional seconds are still accepted and converted the same way. | Not a `lint` rule                     |
